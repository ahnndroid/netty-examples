...
import discard.server.handler.DiscardServerHandler;
...
public class DiscardServer {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			// (2-4) 클라이언트로부터의 연결 채널에 대한 초기화를 위한 설정
			   .childHandler(new ChannelInitializer<SocketChannel>() {
				   ...
				   protected void initChannel(SocketChannel ch) throws Exception {
					   ...
					   cpip.addLast(new DiscardServerHandler());	// 꽂은 파이프라인을 통해 들어온 이벤트 데이터를 처리할 이벤트 핸들러를 플러그인
				   }
			   });
			 ...
			 */
			// 3-1) Discard 서버가 특정 포트를 사용하기 위한 포트 바인딩 호출 (이 때, 포트 바인딩은 비동기로 수행)
